...
import { buildSuggestedPlanner, normalizeWeeklyCompletionRecords, resolveWeek, type CompletionLog, type PlannerState, type ResolvedWorkout, type WeeklyCompletionRecord } from '../utils/planning';
...
function compactWorkoutLabel(workout: ResolvedWorkout) { const title = workout.type === 'race' ? `🏁 ${formatRaceLabel(workout)}` : cleanLabel(workout.title); const duration = workout.plannedDurationMin && workout.type !== 'race' ? ` ${workout.plannedDurationMin} min` : ''; return `${title}${duration}${workout.status === 'completed' ? ' ✓' : ''}`; }
