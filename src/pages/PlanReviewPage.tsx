...
import { buildSuggestedPlanner, resolveWeek, suggestedDateForWorkout, type CompletionLog, type PlannerState } from '../utils/planning';
...
export function PlanReviewPage() {
  ...
  return <PageStack>
    ...
    <CardStack>{plan.weeks.map((week) => <WeekReviewCard key={week.id} week={week} planner={planner} logs={logs} onOpen={() => setSelectedWeek(week)} />)}</CardStack>
    <SlidePanel isOpen={Boolean(selectedWeek)} title={selectedWeek ? `Week ${selectedWeek.weekNumber}` : 'Week details'} subtitle={selectedWeek ? `${formatPhase(selectedWeek.phase)} • ${formatWeekType(selectedWeek.weekType)} • ${formatDate(selectedWeek.startsOn)} – ${formatDate(selectedWeek.endsOn)}` : undefined} onClose={() => setSelectedWeek(null)}>{selectedWeek ? <WeekDetail week={selectedWeek} planner={planner} logs={logs} onAssign={(id, date) => savePlanner({ ...planner, assignments: { ...planner.assignments, [id]: date } })} /> : null}</SlidePanel>
  </PageStack>;
}

function WeekReviewCard({ week, planner, logs, onOpen }: { week: GeneratedTrainingWeek; planner: PlannerState; logs: CompletionLog[]; onOpen: () => void }) {
  ...
  return <button type="button" onClick={onOpen} style={{ border: 0, padding: 0, background: 'transparent', textAlign: 'left', width: '100%' }}><SectionCard><CardStack>
    ...
    <SampleWeek workouts={workouts} week={week} planner={planner} />
  </CardStack></SectionCard></button>;
}

function WeekDetail({ week, planner, logs, onAssign }: { week: GeneratedTrainingWeek; planner: PlannerState; logs: CompletionLog[]; onAssign: (id: string, date: string) => void }) {
  const longRun = findLongRun(week);
  const quality = findQuality(week);
  const workouts = resolveWeek(week, planner, logs).workouts;
...
function SampleWeek({ workouts, week, planner, compact = false }: { workouts: Array<GeneratedWorkout & { assignedDay?: string }>; week: GeneratedTrainingWeek; planner: PlannerState; compact?: boolean }) { return <div style={{ display: 'grid', gap: compact ? spacing.xs : spacing.sm }}>{weekDays.map((day, index) => { const date = isoAdd(week.startsOn, index); const dayWorkouts = workouts.filter((workout) => (workout.assignedDay ?? planner.assignments[workout.id]) === date); return <div key={day} style={sampleRowStyle}><span style={{ ...typography.caption, color: colors.neutral.muted, minWidth: 38 }}>{day}</span><span style={{ ...typography.small, color: dayWorkouts.length ? colors.neutral.text : colors.neutral.muted, fontWeight: dayWorkouts.length ? 700 : 400 }}>{dayWorkouts.length ? dayWorkouts.map(w => `${w.type === 'race' ? '🏁 ' : ''}${w.title}${w.status === 'completed' ? ' ✓' : ''}`).join(' + ') : 'Recovery / Rest'}</span></div>; })}</div>; }
...
function WorkoutPanelSection({ title, workouts }: { title: string; workouts: GeneratedWorkout[] }) { return <PanelSection title={title}>{workouts.length ? <div style={{ display: 'grid', gap: spacing.sm }}>{workouts.map((workout) => <div key={workout.id} style={workoutPillStyle}><strong style={{ ...typography.h3 }}>{workout.type === 'race' ? '🏁 ' : ''}{workout.title}</strong><span style={{ fontWeight: 700 }}>{workoutMeasure(workout)} • {workout.intensity}</span><span style={{ color: colors.neutral.muted }}>{workout.purpose}</span></div>)}</div> : <p style={{ ...typography.small, color: colors.neutral.muted, margin: 0 }}>None</p>}</PanelSection>; }
