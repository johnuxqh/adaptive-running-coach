...
export function WeekPlannerPage() {
  ...
  function completeWorkout(workoutId: string, details: { actualDistanceKm?: number; actualDurationMinutes?: number; perceivedEffort?: number; journalNote?: string }) {
    ...
    setFeedback('Nice work. Your reflection has been saved.'); setPanel(null);
  }
  function remove(workoutId: string) { if (isArchived) { setFeedback('Completed weeks are read-only.'); return; } if (logs.some((log) => log.workoutId === workoutId)) { setFeedback('Completed workouts stay locked. Use Edit Reflection instead.'); return; } const { [workoutId]: _removed, ...assignments } = planner.assignments; savePlanner({ ...planner, assignments }); setFeedback('Workout returned to available workouts.'); }
  ...
  return <PageStack>
    <div style={compactHeroStyle}>
      <div>
        ...
        <h2 style={{ ...typography.h2, margin: `${spacing.xxs}px 0 0` }}>Plan Week</h2>
      </div>
...
function WeekSnapshot({ days, todayIso, archived, onDayTap, onWorkoutTap, onMove }: { days: Array<{ name: string; date: string; workouts: ResolvedWorkout[] }>; todayIso: string; archived?: boolean; onDayTap: (day: { name: string; date: string; workouts: ResolvedWorkout[] }) => void; onWorkoutTap: (workout: ResolvedWorkout) => void; onMove: (workout: ResolvedWorkout) => void }) {
  return <SectionCard><CardStack>
    <div><h3 style={{ ...typography.h3, margin: 0 }}>Week Planner</h3><p style={{ ...typography.small, color: colors.neutral.muted, margin: `${spacing.xxs}px 0 0` }}>{archived ? 'This completed week is saved in your training history.' : 'Coach recommendation. Tap a workout for details, or move it to fit life.'}</p></div>
    <div style={{ display: 'grid', gap: spacing.xs }}>
...
function panelTitle(panel: PanelMode, day?: { name: string } | null, workout?: GeneratedWorkout | null) { if (!panel) return ''; if (panel.kind === 'review') return panel.done ? 'Added to your training history' : 'Review Week'; if (panel.kind === 'past') return 'Update past day'; if (panel.kind === 'detail') return workout?.title ?? 'Workout details'; if (panel.kind === 'complete') return 'Complete workout'; if (panel.kind === 'remaining') return 'Available Workouts'; if (panel.kind === 'chooseForDay' || panel.kind === 'day') return day?.name ?? 'Day'; return panel.kind === 'moveWorkout' ? `Move ${workout?.title ?? 'Workout'}` : `Assign ${workout?.title ?? 'Workout'}`; }
...
function WeeklyReviewPanel(props: { week: GeneratedTrainingWeek; workouts: ResolvedWorkout[]; nextWeek?: GeneratedTrainingWeek; archivedRecord?: WeeklyCompletionRecord; reflection: string; setReflection: (value: string) => void; done: boolean; confirm: boolean; onAskConfirm: () => void; onArchive: () => void; onReviewAgain: () => void; onPlanNext: () => void }) {
  ...
  return <CardStack>
    ...
    <section style={{ display: 'grid', gap: spacing.xs }}><label htmlFor="weekly-reflection" style={{ ...typography.h3, margin: 0 }}>Weekly Reflection</label><p style={{ ...typography.small, color: colors.neutral.muted, margin: 0 }}>Looking back on the week... What would you like to remember about this week?</p>{archived ? <p style={{ ...typography.body, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', margin: 0 }}>{archivedRecord?.weeklyReflection || 'No weekly reflection was added.'}</p> : <><textarea id="weekly-reflection" maxLength={1500} value={props.reflection} onChange={(event) => props.setReflection(event.target.value)} placeholder="Work was stressful early in the week, but the Sunday long run restored my confidence." style={journalTextAreaStyle} /><p style={{ ...typography.caption, color: colors.neutral.muted, margin: 0 }}>{props.reflection.length}/1500</p></>}</section>
    {!archived && (props.confirm ? <section style={contextStyle}><h4 style={{ ...typography.h3, margin: 0 }}>Finish this week?</h4><p style={{ ...typography.small, margin: 0 }}>Your schedule and workout records will become part of your training history. The weekly record will remain available to review.</p><PrimaryButton onClick={props.onArchive}>Finish Week</PrimaryButton></section> : <PrimaryButton onClick={props.onAskConfirm}>Finish Week</PrimaryButton>)}
    {archived ? <PrimaryButton onClick={props.onPlanNext}>Plan Week</PrimaryButton> : null}
  </CardStack>;
}
