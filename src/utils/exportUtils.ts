...
import { resolveWeek, type CompletionLog, type PlannerState } from './planning';

export type { PlannerState } from './planning';
...
export function buildFullPlanCsv(plan: GeneratedTrainingPlan, planner: import('./planning').PlannerState = { assignments: {}, extraWorkouts: [] }) {
  const columns = ['athleteName','raceDistance','raceDate','weekNumber','weekStart','weekEnd','phase','weekType','category','workoutTitle','type','plannedDay','plannedDistanceKm','plannedDurationMin','intensity','purpose','warmup','mainSet','cooldown','coachTip'];
  const rows = plan.weeks.flatMap((week) => resolveWeek(week, planner, []).workouts.map((workout) => ({ athleteName: plan.summary.athleteName, raceDistance: plan.summary.raceDistance, raceDate: plan.summary.raceDate, weekNumber: week.weekNumber, weekStart: week.startsOn, weekEnd: week.endsOn, phase: week.phase, weekType: week.weekType, category: workout.category, workoutTitle: workout.title, type: workout.type, plannedDay: workout.assignedDay ?? 'Unplanned', plannedDistanceKm: workout.plannedDistanceKm ?? '', plannedDurationMin: workout.plannedDurationMin ?? '', intensity: workout.intensity, purpose: workout.purpose, warmup: workout.warmup, mainSet: workout.mainSet, cooldown: workout.cooldown, coachTip: workout.coachTip })));
  return csv(rows, columns);
}

export function buildCoachReportCsv(plan: GeneratedTrainingPlan, planner: import('./planning').PlannerState, logs: WorkoutLog[], summaries: WeekSummary[]) {
